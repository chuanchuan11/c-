...
```
a) Flyweight: 抽象享元角色
    所有具体享元类的父类, 规定接口，可以接收并作用于外部状态

b) ConcreteFlyweight: 具体享元角色
    抽象享元角色的具体实现类, 实现规定得接口, 并定义一些内部状态

c) unsharedConcreteFlyweight: 非共享得享元角色
    flyweight接口使共享成为可能,但它不是强制的, 也可以创建非共享的角色

c) FlyweightFactory: 享元工厂角色
    创建并管理flyweight对象, 当用户请求一个flyweight时，提供一个已创建的实例, 如果请求的实例不存在，就新创建一个
